...
def sequence_multilabel_loss_with_logits(
    ...
    if label_smoothing is not None and label_smoothing > 0.0:
        ...
        resulting_loss = loss(logits_flat.float(), targets_flat.float())
    # shape : (batch, sequence_length)
    # likelihood = likelihood_flat.view(*targets.size())
    # shape : (batch, sequence_length)
    # likelihood = likelihood * weights
    """
    if average == "batch":
        # shape : (batch_size,)
        per_batch_loss = likelihood.sum(non_batch_dims) / (
            weights_batch_sum + tiny_value_of_dtype(likelihood.dtype)
        )
        ...
        return likelihood.sum() / (
            weights_batch_sum.sum() + tiny_value_of_dtype(likelihood.dtype)
        )
    else:
        # shape : (batch_size,)
        per_batch_loss = likelihood.sum(non_batch_dims) / (
